...
// In a browser, it waits for the page to load. In an app, it waits for the phone to be ready.
...
// Nothing exists outside of this, which prevents all loading errors.
...
function main() {
    ...
    const ads = {
        ...
        initialize: function() {
            ...
            if (typeof admob === 'undefined') { console.log("AdMob plugin not found. This is normal in a browser."); return; }

            this.isReady = true;

            // YOUR REAL AD IDs ARE HERE

            const adIds = {

                banner: 'ca-app-pub-6989088248302599/1900301188',

                reward: 'ca-app-pub-6989088248302599/1842760742',

            };

            admob.banner.config({ id: adIds.banner, autoShow: false });

            admob.rewardvideo.config({ id: adIds.reward, autoShow: false });
        ...
        showBanner: function() { if (this.isReady) admob.banner.show(); },

        hideBanner: function() { if (this.isReady) admob.banner.hide(); },

        showRewardedVideo: function() {

            if (!this.isReady) { console.log("Ads not ready."); return Promise.reject('Ads not ready'); }

            return new Promise((resolve) => {

                document.addEventListener('admob.rewardvideo.events.REWARD', function listener(event) {
    ...
    ui.getCoinsBtn.addEventListener('click', () => { ads.showRewardedVideo().then(success => { if (success) { coins += 500; saveProgress(); updateUI(); renderShop(); } }); });
